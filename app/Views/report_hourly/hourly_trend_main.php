        ...
        <div class="card p-1">
          <form id="main">
            <div class="d-flex align-items-start align-items-end">
              <div class="p-1">
                <label>Date</label>
                <select name="d" class="form-control form-control-sm" onchange="hideAll()" style="width: auto;">
...
<script>
  ...
  function getOption() {
    ...
    $.ajax({
      url: "<?= $this->BASE_URL ?>Report_Hourly/option_list",
  ...
  function trend_data(tc) {
    ...
    $.ajax({
      url: "<?= $this->BASE_URL ?>Report_Hourly/trend_data/" + tc + "/" + base,
      data: $("form#main").serialize(),
      type: "POST",
